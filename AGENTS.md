...
- 当前状态特征包含占用/速度网格、当前相位、持续时间、剩余时间、相位压力、全局等待/延误统计和逐车道车辆/排队/等待统计；仍未包含历史趋势。
- reward 权重尚未经过平台训练调优。
...
- 设计固定维度特征，包括 14 条进口车道的占用/速度网格、相位、剩余时间、相位压力、交通统计和逐车道统计。
...
- 当前 `Config.DIM_OF_OBSERVATION = 618`，其中 560 维为 14 条进口车道乘 20 个格子的占用和速度，8 维为信号相位与持续时间特征，8 维为相位压力与全局交通统计，42 维为逐车道统计。
- 交通统计特征包括 4 个归一化相位压力、进口车辆数、排队比例、平均等待时间和平均延误。
- 逐车道统计特征包括 14 条进口车道的车辆数、排队数和平均等待时间。
- 如增加历史趋势等特征，必须同步修改 `Config.DIM_OF_OBSERVATION` 和模型输入层。
...
7. 如基础 Target-DQN 稳定，再考虑增加历史趋势特征或改成 80 维联合动作。
