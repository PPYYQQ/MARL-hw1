...
- P03 平台 PPO 重跑结果位于 `ppo3/`：任务 ID `210257`，约 15min 后失败；代码包内 `agent_ppo/` 与当前本地一致，已排除 PPO 主线漏传，但 learner trainer 仍在约 5 秒后 `signal_killed`，下一步必须获取平台 `trainer` 文件 ERROR/ALL 日志。
...
- `agent_dqn`、`agent_diy` 仍基本保留模板状态，不是当前主线；`agent_ppo` 是备选线，P03 已确认最新 `agent_ppo/` 仍停在 learner trainer 启动阶段，继续处理前需要平台 `trainer` 细日志。
