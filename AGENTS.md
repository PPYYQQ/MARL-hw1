...
- `agent_target_dqn` 是已平台短训验证的主线算法包；`agent_ppo` 已补齐 PPO loss、entropy、GAE、非零 reward、638 维特征、20 桶 duration、checkpoint 容错、workflow 容错、周期性片段样本发送和片段末尾 value bootstrap，但仍未平台训练验证；`agent_dqn`、`agent_diy` 仍应视为平台模板/备选代码。
...
- P03 平台 PPO 重跑结果位于 `ppo3/`：任务 ID `210257`，约 15min 后失败；代码包内 `agent_ppo/` 与当前本地一致，已排除 PPO 主线漏传，但 learner trainer 仍在约 5 秒后 `signal_killed`；用户确认平台只显示 `aisrv`、`learner`、`env` 日志，因此已在 PPO 启动阶段加入 `[PPO_DIAG]` 和 `faulthandler` 诊断。

仍需关注的问题：

- E06 已验证 phase-bias 调参有效，短训指标明显改善；当前不建议立刻改代码，优先用同一 E06 包跑 2-3h 长训或正式评估，确认 score 是否稳定在 `1100+`、平均延误是否维持在 `20-25`。
...
- `agent_dqn`、`agent_diy` 仍基本保留模板状态，不是当前主线；`agent_ppo` 是备选线，P03 已确认最新 `agent_ppo/` 仍停在 learner trainer 启动阶段；如继续 PPO，应上传带 `[PPO_DIAG]` 的诊断包并看 learner 日志。
...
9. 如 E06 长训/评估仍 phase 2 偏置明显且成绩不稳，再优先下载真实 observation 样例核对四相位压力字段；确认字段无误后再考虑加入更强的规则先验或相位轮转约束。
