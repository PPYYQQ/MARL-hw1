...
duration reward 的目标时长也与模型动作空间保持一致。虽然环境配置允许 `MAX_GREEN_DURATION=40`，当前联合动作头只能表达 `8-27` 秒，因此 reward 中的目标 duration 会限制到同一范围，避免高压场景下模型被惩罚去追一个无法输出的 40 秒动作。
