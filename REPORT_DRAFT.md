...
相位时间特征、相位服务年龄、reward 公平性项和 workflow 帧号读取也使用有限值清洗；异常 `phase_id`、`duration`、`remaining_duration`、`frame_no` / `frameNo` 或旧相位服务记录会回退为保守默认值。workflow 帧号会优先读取顶层字段，也会从 `extra_info` / `_state` / `state` / `info` 中回退读取，适配平台把帧号放在额外信息对象里的返回形态。
